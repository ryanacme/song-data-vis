  ...
  Template.song_feature_list.helpers({
    "get_all_feature_values":function(){
      if (Session.get("feature") != undefined){        
        ...
        // var test = Songs.findOne().Session.get("feature")["type"].Session.get("feature")["name"];
        // console.log(test);
        ...
        songs.forEach(function(song){
            // console.log(song.metadata.tags);
            features[ind] = {
              ...
              // status: (song._id == Session.get("selected_item")),
              id:song._id,
            };
            ind ++;
        })
...
function initDateVis(){
  ...
  songs.forEach(function(song){
    if (song.metadata.tags.date != undefined && //as long as something is in the array...
      ...
      var label = "ind: "+ind; // then we generate a label for it.
      if (song.metadata.tags.title != undefined){// as long as we have a title
